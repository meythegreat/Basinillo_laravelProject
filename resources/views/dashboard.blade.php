<x-layouts.app.sidebar :title="'Music Dashboard'">
    <div class="mx-auto max-w-7xl p-6 text-black dark:text-white">
        ...
        <section class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
            ...
            {{-- Total Songs --}}
            <div class="p-4 rounded-lg bg-white shadow-sm flex items-center gap-3">
        ...
        <div class="rounded-lg bg-white p-4">
            ...
            @csrf
            <input name="title" placeholder="Title" required class="w-full rounded border bg-gray/10 p-2" />
            <input name="artist" placeholder="Artist" class="w-full rounded border bg-gray/10 p-2" />
            <select name="genre_id" class="w-full rounded border bg-gray/10 p-2">
                <option value="">-- Genre (optional) --</option>
                @foreach($genres as $g) <option value="{{ $g->id }}">{{ $g->name }}</option> @endforeach
            </select>

            <input name="duration_minutes" type="number" min="0" placeholder="Minutes" class="w-full rounded border bg-gray/10 p-2" />
            <input name="duration_seconds" type="number" min="0" max="59" placeholder="Seconds" class="w-full rounded border bg-gray/10 p-2" />
            <input name="release_year" type="number" min="1900" max="2100" placeholder="Release Year" class="w-full rounded border bg-gray/10 p-2" />
                ...
                @forelse($songs as $song)
                    ...
                    <td class="px-4 py-3">
                        ...
                        <div x-show="open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                            ...
                            <h3 class="text-lg font-semibold mb-3 text-white">Edit Song</h3>
                            <form action="{{ route('songs.update', $song) }}" method="POST" class="grid gap-2">
                            @csrf
                            @method('PUT')

                            <input name="title" x-model="title" required class="w-full rounded p-2 bg-white" />
                            <input name="artist" x-model="artist" class="w-full rounded p-2 bg-white" />
                            <select name="genre_id" x-model="genre" class="w-full rounded p-2 bg-white">
                                <option value="">-- Genre (optional) --</option>
                                @foreach($genres as $g) <option value="{{ $g->id }}">{{ $g->name }}</option> @endforeach
                            </select>

                            <div class="grid grid-cols-2 gap-2">
                                <input name="duration_minutes" type="number" min="0" x-model.number="minutes" placeholder="Minutes" class="w-full rounded p-2 bg-white" />
                                <input name="duration_seconds" type="number" min="0" max="59" x-model.number="seconds" placeholder="Seconds" class="w-full rounded p-2 bg-white" />
                            </div>

                            <input name="release_year" type="number" min="1900" max="2100" x-model="year" class="w-full rounded p-2 bg-white" />
                            <textarea name="notes" x-model="notes" class="w-full rounded p-2 bg-white" rows="2"></textarea>

                            <div class="flex gap-2 mt-2">
                                <button type="button" @click="open=false" class="px-3 py-1 rounded border text-white bg-red-500">Cancel</button>
                                <button type="submit" class="ml-auto px-4 py-1 rounded bg-green-400 border">Save</button>
                            </div>
                            </form>
                            </div>
                        </div>
